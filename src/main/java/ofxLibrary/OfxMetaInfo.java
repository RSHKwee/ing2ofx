...
public class OfxMetaInfo {
	...
	public OfxMetaInfo(String a_bankcode, File a_SynonymesFile) {
		bankcode = a_bankcode;
		m_Synonyms = new Synonyms(a_SynonymesFile);
	...
	public String getPrefix() {
		...
		if (!prefix.isEmpty()) {
			...
			LOGGER.log(Level.FINE, "Search synonyme for account: " + account);
			String ll_prefix = m_Synonyms.getSynonym(account);
			if (!ll_prefix.isEmpty()) {
				l_prefix = ll_prefix;
				LOGGER.log(Level.FINE, "Found synonyme for account: " + account + " found: " + ll_prefix);
			}
		}
